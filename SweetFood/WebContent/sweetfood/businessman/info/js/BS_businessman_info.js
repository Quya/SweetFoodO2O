		...
		bindData:function (modelObj){
		   ...
		   if (modelObj!=null) {			  
		        ...
		        for (var item in modelObj) {
		        	if(item=="shop"&&modelObj[item]!="")
		        	{
		        		alert(modelObj[item]);
		        		viewModel["prov"]=  ko.observable(modelObj[item]["address"].province);
		        		viewModel["city"]=ko.observable(modelObj[item]["address"].city);
		        		viewModel["dist"]=ko.observable(modelObj[item]["address"].dist);
		        		viewModel["street"]=ko.observable(modelObj[item]["address"].street);
		        		viewModel["addressid"]=ko.observable(modelObj[item]["address"].addressId);
		        		viewModel["indroduce"]=ko.observable(modelObj[item].indroduce);
		        		if(modelObj[item]["address"].province!=undefined)
		        		{
		        			$(function(){
		        				$("#city").citySelect({
		        					prov:modelObj[item]["address"].province,
		        					city:modelObj[item]["address"].city,
		        					dist:modelObj[item]["address"].dist,
		        					nodata:"none",
		        					required:false
		        				}); 
		        			});
		        	...
		        	}else if(item=="shop" && modelObj[item]==""){
		        		viewModel["prov"]=  ko.observable("");
		        		viewModel["city"]=ko.observable("");
		        		viewModel["dist"]=ko.observable("");
		        		viewModel["street"]=ko.observable("");
		        		viewModel["addressid"]=ko.observable("");
		        		viewModel["indroduce"]=ko.observable("");
	        		}
		        	if(item=="sex"){
		 ...
		 */
		   saveInfo:function (){
			   ...
			   if(imgName==""||(imgName!=""&&(imgName.indexOf(".png")>-1||imgName.indexOf(".jpg")>-1||imgName.indexOf(".gif")>-1||imgName.indexOf(".jpeg")>-1))){
					AT_businessman_info.saveInfo("?id="+$("#userid")[0].value+"&username="+$("#username")[0].value+"&email="+$("#email")[0].value+"&phone="+$("#phone")[0].value+"&prov="+prov+"&city="+city
						+"&dist="+dist+"&street="+street+"&addressId="+$("#addressid")[0].value+"&indroduce="+$("#shopdescription")[0].value+"&sex="+sex+"&power="+$("#power")[0].value);	

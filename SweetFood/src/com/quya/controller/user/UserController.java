...
public class UserController extends BaseJsonController{
	...
	public String login(HttpServletRequest request, HttpServletResponse response, RedirectAttributes attr){
		...
		User user=new User();
		user.setEmail(reqJson.get("loginname").toString());
		user.setPassword(reqJson.get("password").toString());
		 try{
			 userService.login(user);
			 request.getSession().setAttribute("user", user);
			 sendMessageForObject(response, "1", user);
	...
	public String logout(HttpServletRequest request, HttpServletResponse response, RedirectAttributes attr){
        ...
        sendMessageForObject(response, "1", "登出成功");
	...
	public String regist(HttpServletRequest request, HttpServletResponse response, RedirectAttributes attr){
		...
		JSONObject reqJson = getJSONParams(request);		 
		 ...
		 user.setPhotoUrl("/SweetFood/sweetfood/common/image/default.jpg");
